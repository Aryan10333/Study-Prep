...
"""
    ...
    #
    # virtual-time-budget set to 30000ms (not the older 8000ms default): a prior
    # topic's math-heavy interview cheatsheet failed all 3 retries identically at
    # 8000ms (not a transient race -- every attempt produced 0 bytes) because KaTeX
    # auto-render needed more time to finish before print-to-pdf fired. This topic
    # has formula-bearing modules too (02, 04, 08) plus 8 inline SVG diagrams, so the
    # higher budget is used from the start rather than discovered the hard way again.
    ...
    # was never actually regenerated -- confirmed in practice: a re-compile after editing
    # source content reported SUCCESS while the PDF's mtime proved it was untouched.
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        # practice (this session): Edge's own background-mode/startup-boost service
        # respawns a shadow process within seconds of being killed, and ANY existing
        # msedge.exe process (not just orphaned debug artifacts) makes a new headless
        # launch silently forward to that "existing browser session" instead of running
        # independently -- print-to-pdf then exits 0 with no file written, no error, no
        # output (even `msedge --version` prints nothing when this happens). Killing
        # once before the whole retry loop isn't enough, since reading/building the HTML
        # (done once, before this loop) already gives the service time to respawn.
        # Explicit, user-approved trade-off: this closes any Edge windows/tabs open at
        # this exact moment, every attempt. See pdf_compiler/SKILL.md Section 4.
...
def main():
    ...
    print("\n--- Compiling Master AI Agents & Protocols Study Guide ---")
    master_html = os.path.join(base_dir, "ai_agents_master_study_guide.html")
    master_pdf = os.path.join(base_dir, "ai_agents_master_study_guide.pdf")

    master_modules = [
        "01_agent_fundamentals_and_reasoning.md",
        "02_tool_calling_and_function_calling.md",
        "03_mcp_and_agent_tool_standardization.md",
        "04_context_state_and_memory.md",
        "05_orchestration_and_durable_execution.md",
        "06_multi_agent_systems.md",
        "07_agent_frameworks_landscape.md",
        "08_agent_evaluation_and_debugging.md",
        "09_production_agents_safety_and_security.md",
    ]
    compile_document(
        master_modules,
        master_html,
        master_pdf,
        "AI Agents, Tool Calling & Protocols",
        "AI Agents & Protocols Curriculum Guide",
        subtitle="Reasoning Patterns, Tool Calling, MCP, Memory, Durable Orchestration, Multi-Agent Systems &amp; Production Safety",
        focus_areas="ReAct, tool-schema design, MCP security, Context/State/Memory, durable execution, multi-agent coordination, agent evaluation, indirect prompt injection",
        includes="Step-by-Step Numerical Hand Calculations, Python Implementations, SVG/HTML Diagrams",
    )

    print("\n--- Compiling AI Agents & Protocols Interview Q&A Cheatsheet ---")
    qa_html = os.path.join(base_dir, "ai_agents_interview_cheatsheet.html")
    qa_pdf = os.path.join(base_dir, "ai_agents_interview_cheatsheet.pdf")

    compile_document(
        ["10_ai_agents_interview_questions.md"],
        qa_html,
        qa_pdf,
        "AI Agents & Protocols Interview Q&A Cheatsheet",
        "AI Agents & Protocols — 59 Interview Questions & Answers",
        subtitle="Real Measured Results from This Topic's Own Executed Notebooks, Grounded in Every Module's Hand Calculations",
        focus_areas="Agent fundamentals & reasoning, tool calling, MCP, context/state/memory, durable orchestration, multi-agent coordination, frameworks landscape, agent evaluation, production safety & security",
