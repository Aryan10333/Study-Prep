...
- **Token Merges**: The BPE merge loop checks adjacent characters, identifies the most frequent pair (e.g. `('g', 'u')` or `('a', 'g')`), and merges them to form a new subword token.
- **Vocabulary Expansion**: The initial vocab of single characters grows by 5 tokens corresponding to the most frequent character combinations, illustrating how modern tokenizers (like Llama and GPT) represent frequent subword units.
"""))
...
- **Smoothed IDFs**: Computed using $\log((1 + N)/(1 + \text{DF})) + 1$. This scales down highly frequent terms (like `"to"`) while amplifying rare terms.
...
- **Exact Alignment**: The assertion passes successfully with `atol=1e-5`, verifying that our mathematical derivation and coding from scratch matches Scikit-Learn's `TfidfVectorizer` implementation exactly.
"""))
...
- **Corpus**: We loaded the raw sentences of Lewis Carroll's *Alice in Wonderland* locally via NLTK's `gutenberg` reader to ensure offline execution.
...
- **Models**: We trained a **CBOW** model (which predicts the target word given its surrounding context) and a **Skip-gram** model (which predicts context words given the target word).
...
- **FastText Success**: FastText decomposes `"alicean"` into character n-grams (`ali`, `lic`, `ice`, `cea`, `ean`) and sums their representations to synthesize a vector, resolving the query with a high semantic similarity to `"alice"`.
"""))
...
"""))
    ...
    # Markdown Explanation 1
    cells.append(nbf.v4.new_markdown_cell("""### Output Explanation: Projection Matrices
- **Tensors**: Initializes $Q$, $K$, and $V$ matrices of shape `(4, 128)`, representing a 4-token sequence projected into a 128-dimensional attention space.
"""))
    
    # Code Cell 2: Unscaled calculations
    cells.append(nbf.v4.new_code_cell(r"""# Unscaled dot product attention
...
"""))
    ...
    # Markdown Explanation 2
    cells.append(nbf.v4.new_markdown_cell("""### Output Explanation: Unscaled Attention
- **Scores**: Without scaling, the dot products are large because variance scales as $O(d_k)$.
- **Saturation**: Softmax is highly saturated (containing values near $1.0$ or $0.0$), producing extremely small gradients.
"""))
    
    # Code Cell 3: Scaled calculations
    cells.append(nbf.v4.new_code_cell(r"""# Scaled Attention (dividing by sqrt(d_k))
...
"""))
    ...
    # Markdown Explanation 3
    cells.append(nbf.v4.new_markdown_cell("""### Output Explanation: Scaled Attention
- **Variance Control**: Dividing by $\sqrt{128} \approx 11.31$ scales the score variance back to $1.0$.
- **Preserved Sensitivity**: The Softmax distribution is smoother and more sensitive, ensuring healthy gradient flow during backpropagation.
"""))
